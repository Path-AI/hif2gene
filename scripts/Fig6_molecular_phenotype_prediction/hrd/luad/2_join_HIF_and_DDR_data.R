...
luad.hifs.with.ddr.scores <- readRDS(file="data/genetic_data/HRD/processed/luad.hifs.with.ddr.scores.rds")
luad.hifs.with.ddr.scores <- dplyr::select(luad.hifs.with.ddr.scores, bcr_patient_barcode, everything()) # Key of interest = TCGA barcode, move to first column of dataframe

# Load in DDR scores data (filtered by TCGA barcodes) 
ddr.scores.luad <- readRDS(file="data/genetic_data/HRD/processed/ddr.scores.luad.rds")
...
luad.barcodes <- luad.hifs.with.ddr.scores$bcr_patient_barcode 
length(luad.barcodes[duplicated(luad.barcodes)]) # Duplicated barcodes: same patient, different samples
luad.hifs.with.ddr.scores <- luad.hifs.with.ddr.scores %>% group_by(bcr_patient_barcode) %>% filter(`AREA (MM2) OF [TUMOR] IN [TISSUE]_HE`==max(`AREA (MM2) OF [TUMOR] IN [TISSUE]_HE`))
dim(luad.hifs.with.ddr.scores)
# Second filter, if tumor areas are equal, choose randomly
luad.hifs.with.ddr.scores <- luad.hifs.with.ddr.scores %>% group_by(bcr_patient_barcode) %>% sample_n(1)
dim(luad.hifs.with.ddr.scores)

# Join HIF and DDR datasets
joined <- dplyr::left_join(luad.hifs.with.ddr.scores, ddr.scores.luad, by="bcr_patient_barcode")
...
saveRDS(hrd.use, file="data/genetic_data/HRD/datasets/luad.HRD_Score.joined.USE.rds")
write.csv(hrd.use, file="data/genetic_data/HRD/datasets/luad.HRD_Score.joined.USE.csv")
saveRDS(hrd.hold.out, file="data/genetic_data/HRD/datasets/luad.HRD_Score.joined.HOLDOUT.rds")
write.csv(hrd.hold.out, file="data/genetic_data/HRD/datasets/luad.HRD_Score.joined.HOLDOUT.csv")
