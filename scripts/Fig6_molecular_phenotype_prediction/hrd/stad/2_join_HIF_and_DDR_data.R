...
stad.hifs.with.ddr.scores <- readRDS(file="data/genetic_data/HRD/processed/stad.hifs.with.ddr.scores.rds")
stad.hifs.with.ddr.scores <- dplyr::select(stad.hifs.with.ddr.scores, bcr_patient_barcode, everything()) # Key of interest = TCGA barcode, move to first column of dataframe

# Load in DDR scores data (filtered by TCGA barcodes) 
ddr.scores.stad <- readRDS(file="data/genetic_data/HRD/processed/ddr.scores.stad.rds")
...
stad.barcodes <- stad.hifs.with.ddr.scores$bcr_patient_barcode 
length(stad.barcodes[duplicated(stad.barcodes)]) # Duplicated barcodes: same patient, different samples
stad.hifs.with.ddr.scores <- stad.hifs.with.ddr.scores %>% group_by(bcr_patient_barcode) %>% filter(`AREA (MM2) OF [TUMOR] IN [TISSUE]_HE`==max(`AREA (MM2) OF [TUMOR] IN [TISSUE]_HE`))
dim(stad.hifs.with.ddr.scores)
# Second filter, if tumor areas are equal, choose randomly
stad.hifs.with.ddr.scores <- stad.hifs.with.ddr.scores %>% group_by(bcr_patient_barcode) %>% sample_n(1)
dim(stad.hifs.with.ddr.scores)

# Join HIF and DDR datasets
joined <- dplyr::left_join(stad.hifs.with.ddr.scores, ddr.scores.stad, by="bcr_patient_barcode")
...
saveRDS(hrd.use, file="data/genetic_data/HRD/datasets/stad.HRD_Score.joined.USE.rds")
write.csv(hrd.use, file="data/genetic_data/HRD/datasets/stad.HRD_Score.joined.USE.csv")
saveRDS(hrd.hold.out, file="data/genetic_data/HRD/datasets/stad.HRD_Score.joined.HOLDOUT.rds")
write.csv(hrd.hold.out, file="data/genetic_data/HRD/datasets/stad.HRD_Score.joined.HOLDOUT.csv")
