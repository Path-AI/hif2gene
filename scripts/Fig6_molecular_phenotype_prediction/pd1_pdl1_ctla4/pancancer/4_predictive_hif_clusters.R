...
# DEFINE #
##########
outcome.of.interest <- 'PDL1'

# Load in ensemble model coefficients = mean(model1, model2, model3) beta coefficients
df <- read.table(sprintf("data/model_outputs/pd1_pdl1_ctla4/coefficients/pancancer/%s_model_ensemble.csv", outcome.of.interest), sep=",", header=TRUE)
